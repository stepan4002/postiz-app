...
import { useAyrShareConfig } from './hooks/use-ayrshare-config';

export const AyrShareConfigPanel: FC = () => {
  const fetch = useFetch();
  const toaster = useToaster();
  const { data: config, mutate, isLoading } = useAyrShareConfig();
  ...
  const saveConfig = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/ayrshare/config', {
        method: 'POST',
        body: JSON.stringify({ apiKey, planType, maxProfiles }),
        headers: { 'Content-Type': 'application/json' },
      });
      if (res.ok) {
        toaster.show('AyrShare configuration saved successfully', 'success');
        setApiKey('');
        mutate();
      } else {
  ...
  const verifyKey = useCallback(async () => {
    ...
    setVerified(null);
    try {
      const res = await fetch('/ayrshare/config/verify', {
        method: 'POST',
      ...
      if (data.valid) {
        toaster.show('AyrShare API key is valid', 'success');
      } else {
        toaster.show('AyrShare API key is invalid', 'warning');
      }
    } catch {
      setVerified(false);
  ...
  const deleteConfig = useCallback(async () => {
    try {
      const res = await fetch('/ayrshare/config', { method: 'DELETE' });
      if (res.ok) {
        toaster.show('AyrShare configuration removed', 'success');
        mutate();
      }
    } catch {
  ...
  return (
    <div className="bg-third rounded-xl p-6 border border-tableBorder">
      <h3 className="text-lg font-semibold mb-4">AyrShare API Configuration</h3>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-1">
            {config ? 'Update API Key' : 'AyrShare API Key'}
          </label>
          ...
          <div className="flex-1">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value === 'launch') setMaxProfiles(10);
                else if (e.target.value === 'business') setMaxProfiles(25);
                else if (e.target.value === 'enterprise') setMaxProfiles(100);
              }}
              className="w-full bg-input text-inputText rounded-lg px-3 py-2 text-sm border border-tableBorder focus:border-primary outline-none"
            >
              <option value="launch">Launch (10 profiles)</option>
              <option value="business">Business (25 profiles)</option>
              <option value="enterprise">Enterprise (100 profiles)</option>
            </select>
          </div>
          <div className="flex-1">
        ...
        <p className="text-xs text-inputText">
          ...
          . AyrShare supports 13 platforms: Facebook, Instagram, X, LinkedIn, YouTube, TikTok, Pinterest, Reddit, Threads, Bluesky, Telegram, Google Business, and Snapchat.
        </p>
