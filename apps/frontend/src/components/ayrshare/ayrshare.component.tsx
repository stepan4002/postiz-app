...
import { AyrShareConfigPanel } from './config-panel.component';
import { AyrShareProfileList } from './profile-list.component';
import { useAyrShareConfig } from './hooks/use-ayrshare-config';

type Tab = 'profiles' | 'config';

export const AyrShareComponent: FC = () => {
  const [activeTab, setActiveTab] = useState<Tab>('profiles');
  const { data: config } = useAyrShareConfig();

  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full">
      <div>
        <h1 className="text-2xl font-bold">AyrShare Gateway</h1>
        <p className="text-sm text-inputText mt-1">
          Unified social media management via AyrShare API. Manage posting, comments, DMs, and analytics across 13 platforms.
        </p>
      </div>
      ...
      {!config && activeTab === 'profiles' ? (
        <div className="bg-third rounded-xl p-8 border border-tableBorder text-center">
          ...
          <p className="text-sm text-inputText mb-4">
            You need to configure your AyrShare API key before creating profiles and linking social accounts.
          </p>
          <button
            onClick={() => setActiveTab('config')}
